...
<script>
  $(document).ready(function() {
      ...
      dateFormat: 'dd-mm-yy'
		...
		<div class="page-content">
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					<div class="span12">
						<table width="90%" cellpadding="0" cellspacing="0" border="0" style="text-align:center; float:right;" >
							<tr>
								
								<th width="20%;">Employee ID</th>
								<th width="25%;">Date From</th>
								<th width="23%;">Date To</th>
								<th>&nbsp;</th>
							...
							<form action="<?=$_SERVER['PHP_SELF']?>" method="post">
								...
								<td><input type="text" class="dt_pic" name="dateFrom" required /></td>
								<td><input type="text" class="dt_pic" name="dateTo" required /></td>
							...
							<table class="table table-striped table-bordered table-hover" id="sample_2">						
								...
								<?php
									...
									if(isset($_REQUEST['search']))
									{
										...
										$GetUserSql = "SELECT sas.*, sr.Uid, sr.FirstName, sr.LastName FROM staff_available_shift as sas INNER JOIN hr_staff_registration as sr ON sr.Uid = sas.accept_staffid AND sr.RegistrationNo = '".$regNo."' AND sas.accept_staffid != '' AND sas.timeSheets != '' AND sas.status = 'Unprocessed' AND date BETWEEN '".$dateFrom."' AND '".$dateTo."' ORDER BY sas.id DESC";
										
									}else{
										$GetUserSql = "SELECT sas.*, sr.RegistrationNo, sr.FirstName, sr.LastName FROM staff_available_shift as sas INNER JOIN hr_staff_registration as sr ON sr.Uid = sas.accept_staffid AND sas.accept_staffid != '' AND sas.shiftEndTime < NOW() AND sas.timeSheets != '' AND sas.status = 'Unprocessed' ORDER BY sas.id DESC";
									}
									$GetQuery = mysql_query($GetUserSql) or die(mysql_error());
									while($rowdest = mysql_fetch_array($GetQuery))
									{
										
										$Uid = $rowdest['accept_staffid'];
									...
									<tr class="odd gradeX">
										...
										<td class="hidden-480"><div class="videoWrapper">Start : <?php echo $rowdest['start_time'];?><br/>Finish : <?php echo $rowdest['end_time'];?></div></td>
                                         ...
                                         <td class="hidden-480" style="width:120px; text-align:center;">
											 ...
											 <select class="span9" style="color:#CC3300; background-color:#99e3ff;" tabindex="1" id="stat<?=$rowdest['id']?>" onChange="changestatus(this.value,'<?=$rowdest['id']?>')">
												<option value="Processed" <?=$rowdest['status'] == 'Processed' ? 'selected' : ''?>>Processed</option>
												<option value="Unprocessed" <?=$rowdest['status'] == 'Unprocessed' ? 'selected' : ''?>>Unprocessed</option>
											 </select>
									   ...
									   <tr><td colspan="8"><a href="staffinfo.php?Uid=<?=$Uid?>&payslip" target="_blank" class="btn mini green"><i class="icon-edit"></i> Create a Payslip </a></td></tr>
									 <?php }?>
								</tbody>
							</table>
							</div>
	...
	{
		$.post('ajax/statusCompletedShift.php',{ stat : stat , id : id });
	}
</script>
	<!-- END JAVASCRIPTS -->
</body>
