...
{   
			   ...
			   $timezone = "Asia/Calcutta";
				...
				else
			    {
						...
						echo '<script language="javascript">';
						echo 'window.location="userlist.php?mess=UnSuccessful"';
						echo '</script>';
			    }
}
//Update User
...
{      
				...
				//{
					...
					if($_FILES['image']['name']!=''){
								...
								$unlink_sql = "SELECT UserImage FROM ".TABLE_PREFIX."user_registration WHERE Uid = '".$_REQUEST['oldid']."'";
					...
					$imagename = $oldimg;	
						...
						echo '<script language="javascript">';
						echo 'window.location="userlist.php?mess=SuccessfulUpdate"';
						echo '</script>';
				/*}
				else
			    {
...
<!-- END LightBox View -->
							...
							<table class="table table-striped table-bordered table-hover" id="sample_2">						
								...
								<?php
									...
									while($rowdest = mysql_fetch_array($GetQuery)){
										...
										$phone_final_datas =  implode(',',$phone_final_data);
									?>
									<tr class="odd gradeX">
                                    	<td class="hidden-480">
                                    	<?=$ctn?>
										...
										<td class="hidden-480"><div class="videoWrapper"><?=$phone_final_datas?></div></td>
										<!-- <td class="hidden-480">
											<div class="controls">
                                        ...
                                        <td class="hidden-480">		                                        
                                            <a href="userinfo.php?Uid=<?=$rowdest['Uid']?>" class="btn mini green"><i class="icon-edit"></i> View</a>			
	...
	{	
		...
		if(cnf)
		{
			...
			$.post('ajax/delUser2.php',{ feedid : id, mode : 'single' },
				function(data)
				{//deletecinematic
					$('#tablesec').html(data);
					/************************************ Table JS ************************************/
	...
	{
		$.post('ajax/statususer2.php',{ stat : stat , id : id });
	}
</script>
	<!-- END JAVASCRIPTS -->
</body>
