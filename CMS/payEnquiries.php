...
$GetUserSql = "SELECT * FROM pay_enquires where sl=".$_GET["sl"]." ";
...
$sl=$row['sl'];
$answer = $row['answered'];
$ansDate = date('d/m/Y', strtotime($row['answered_date']));
...
$receipt_no =  $row['receipt_no'];
 ...
 $admin_enquery = mysql_query("INSERT INTO staff_enquery SET
  										receipt_no = '".$receipt_no."',
										reply = '".addslashes($_POST["enquires"])."',
										reply_date = NOW(),
										reply_by = 'admin'");
 	if($admin_enquery){
		header("location:payEnquiries.php?msg=ans&sl=".$sl);
	}
 }
...
?>
	 <tr><td>Staff : </td><td rowspan="2"><?= $enq['reply'] ?><br/><?=date('d F Y H:i:s', strtotime($enq['reply_date']));?></td></tr>
	 <tr><td>&nbsp;</td></tr>
<?php }else{?>
