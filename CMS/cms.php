					...
					<div class="span12">
						...
						<div id="tablesec">
							<table class="table table-striped table-bordered table-hover" id="sample_2">						
								...
								<?php
									$ctn = 1;
									$GetUserSql = "SELECT * FROM ".TABLE_PREFIX."cms ORDER BY id asc";
									...
									<tr class="odd gradeX">
										...
										<td class="hidden-480">
											<div class="videoWrapper"><?=substr(strip_tags(stripslashes($rowdest['cms_page_heading'])),0,35)?></div> 
										</td>
										<!--<td class="hidden-480">
											<div class="videoWrapper"><?=substr(strip_tags(stripslashes($rowdest['cms_page_subheading'])),0,10)?></div> 
										</td>-->
										<td class="hidden-480"><div class="videoWrapper"><?php if($rowdest['cms_pagedes']!='') { ?><?=substr(strip_tags(stripslashes($rowdest['cms_pagedes'])),0,50)?><?php } else { ?>N/A<?php } ?></div></td>
										<td class="hidden-480">		
                                            <a class="btn mini green" data-toggle="modal" href="editcms.php?pageid= <?=$rowdest['id']?>">Edit <i class="icon-edit"></i></a>                            			</td>
                                       </tr>
                                       <?php } 
									$GetUserSql = "SELECT * FROM ".TABLE_PREFIX."cms1 ORDER BY id asc";
									$GetQuery = mysql_query($GetUserSql) or die(mysql_error());
									while($rowdest = mysql_fetch_array($GetQuery))
									{ 			
									   ?>
									 <tr class="odd gradeX">
										...
										<td class="hidden-480">
											<div class="videoWrapper"><?=substr(strip_tags(stripslashes($rowdest['cms_pagetitle'])),0,35)?></div> 
										</td>
										
										<td class="hidden-480"><div class="videoWrapper"><?php if($rowdest['text1']!='') { ?><?=substr(strip_tags(stripslashes($rowdest['text1'])),0,50)?><?php } else { ?>N/A<?php } ?></div></td>
										<td class="hidden-480">		
                                            <a class="btn mini green" data-toggle="modal" href="editcms1.php?pageid= <?=$rowdest['id']?>">Edit <i class="icon-edit"></i></a>                            			</td>
                                       </tr>
								<?php }?>
								<?php  
									$GetUserSql = "SELECT * FROM ".TABLE_PREFIX."cms2 ORDER BY id asc";
									$GetQuery = mysql_query($GetUserSql) or die(mysql_error());
									while($rowdest = mysql_fetch_array($GetQuery))
									{ 			
									   ?>
									 <tr class="odd gradeX">
										...
										<td class="hidden-480"><div class="videoWrapper"><?php if($rowdest['text1']!='') { ?><?=substr(strip_tags(stripslashes($rowdest['text1'])),0,50)?><?php } else { ?>N/A<?php } ?></div></td>
										<td class="hidden-480">		
                                            <a class="btn mini green" data-toggle="modal" href="editcms2.php?pageid= <?=$rowdest['id']?>">Edit <i class="icon-edit"></i></a>                            			</td>
                                       </tr>
								<?php }?>
								</tbody>
							</table>
