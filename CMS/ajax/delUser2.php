...
if($_REQUEST['mode'] == "single")
{
	$feedid = $_REQUEST['feedid'];
		...
		$unlink_sql = "SELECT UserImage FROM ".TABLE_PREFIX."user_registration WHERE Uid = '".$feedid."'";
	...
	$delsingle = "DELETE FROM ".TABLE_PREFIX."user_registration WHERE Uid = '".$feedid."'";
...
if($_REQUEST['mode'] == "selected")
{
	...
	foreach($feedids as $valid)
	{
		...
		//Unlink Old ImageBefore Upload New image
		$unlink_sql = "SELECT UserImage FROM ".TABLE_PREFIX."user_registration WHERE Uid = '".$valid."'";
		$unlink_rs = mysql_query($unlink_sql) or mysql_error();
		$row_unlink = mysql_fetch_array($unlink_rs);
	...
	$delsingle = "DELETE FROM ".TABLE_PREFIX."user_registration WHERE Uid = '".$valid."'";
	mysql_query($delsingle) or die(mysql_error());
	}
}
?>
