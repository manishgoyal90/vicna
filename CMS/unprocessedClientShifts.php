							...
							<table class="table table-striped table-bordered table-hover" id="sample_2">						
								...
								<?php
									...
									if(isset($_REQUEST['search']))
									{
										...
										$GetUserSql = "SELECT sas.*, ur.RegistrationNo, ur.FirstName, ur.LastName, ur.Uid FROM staff_available_shift as sas INNER JOIN book_nurse as bn ON sas.shiftid = bn.sl INNER JOIN hr_user_registration as ur ON ur.Uid = bn.clientId AND ur.RegistrationNo = '".$regNo."' AND sas.accept_staffid != '' AND sas.timeSheets != '' AND sas.clientStatus = 'Unprocessed' AND date BETWEEN '".$dateFrom."' AND '".$dateTo."' ORDER BY sas.id DESC";
										
									}else{
										$GetUserSql = "SELECT sas.*, ur.RegistrationNo, ur.FirstName, ur.LastName FROM staff_available_shift as sas INNER JOIN book_nurse as bn ON sas.shiftid = bn.sl INNER JOIN hr_user_registration as ur ON ur.Uid = bn.clientId AND sas.accept_staffid != '' AND sas.shiftEndTime < NOW() AND sas.timeSheets != '' AND sas.clientStatus = 'Unprocessed' ORDER BY sas.id DESC";
									}
									...
									<tr class="odd gradeX">
									   ...
									   if(isset($_REQUEST['search']))
										{
									   ?>
									    <tr><td colspan="8"><a href="userinfo.php?Uid=<?=$Uid?>&invoice" target="_blank" class="btn mini green"><i class="icon-edit"></i> Create a Payslip </a></td></tr>
									<?php }?>
								</tbody>
							</table>
							</div>
