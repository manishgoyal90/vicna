<?php include"lib/header.php";
		...
		if(isset($_REQUEST['submit']) && isset($_REQUEST['action']) && $_REQUEST['action'] == 'add' ) {  
				...
				echo 'alert("Advertise Send Successfully");';
				echo "window.top.location.href='processBooking.php?sl=".$id."';\n";
				echo "</script>";
				exit();
			...
			$FetchUserSql = "SELECT * FROM book_nurse WHERE sl = '".$sl."'"; 
			
			$FetchUserQuery = mysql_query($FetchUserSql);
			$NumRows =mysql_num_rows($FetchUserQuery);
			$rowdest = mysql_fetch_array($FetchUserQuery);
			
			if($_REQUEST['shift'] == 'first')
			{
				$location = $rowdest['word1'];
				$qualification = $rowdest['class1'];
				$role = $rowdest['speciality1'];
				$date = $rowdest['firstShiftDate'];
				$start_time = $rowdest['startTime1'];
				$end_time = $rowdest['finishTime1'];
			}elseif($_REQUEST['shift'] == 'second')
			{
				$location = $rowdest['word2'];
				$qualification = $rowdest['class2'];
				$role = $rowdest['speciality2'];
				$date = $rowdest['secondShiftDate'];
				$start_time = $rowdest['startTime2'];
				$end_time = $rowdest['finishTime2'];
			}elseif($_REQUEST['shift'] == 'third')
			{
				$location = $rowdest['word3'];
				$qualification = $rowdest['class3'];
				$role = $rowdest['speciality3'];
				$date = $rowdest['thirdShiftDate'];
				$start_time = $rowdest['startTime3'];
				$end_time = $rowdest['finishTime3'];
			}
					...
					<div class="span12">
						...
						<ul class="breadcrumb">
						 <li>
						  ...
						  <li>
							<a href="processBooking.php?sl=<?=$sl?>">Available Shift List</a> 
							<span class="icon-angle-right"></span>
						 </li>
			...
			<!-- BEGIN PAGE CONTENT-->
				<div class="row-fluid">
					...
					<form name="cmspageform" id="cmspageform" class="form-horizontal" action="<?=$_SERVER['PHP_SELF']?>" method="POST" style="margin:0; padding:0" enctype="multipart/form-data" onSubmit="return check();">
							...
							<input type="hidden" name="id" id="id" value="<?=$sl?>">
							<input type="hidden" name="shift" value="<?=$_REQUEST['shift'];?>" />
							<input type="hidden" name="clientId" value="<?=$rowdest['clientId'];?>" />
                               ...
                               <div class="span12 ">
                                  ...
                                  <div class="control-group">
                                     <label class="control-label">Location</label>
									  <div class="controls">
										 <input type="text" class="span8 m-wrap" name="location" id="location" value="<?=$location;?>" />
									  </div>
                                  </div>
                                   ...
                                   <div class="control-group">
                                     <label class="control-label">Qualification</label>
									  <div class="controls">
									  	<input type="text" class="span8 m-wrap" name="qualification" id="qualification" value="<?=$qualification?>" />
								  ...
								  <div class="control-group">
                                     <label class="control-label">Speciality</label>
									  <div class="controls">
									  	<input type="text" class="span8 m-wrap" name="role" id="role" value="<?=$role;?>" />
                                   ...
                                   <div class="control-group">
                                     <label class="control-label">Date</label>
									  <div class="controls">
										 <input type="date" class="span8 m-wrap" name="date" id="date" value="<?=$date;?>" />
									  </div>
                                  </div>
                                   <div class="control-group">
                                     <label class="control-label">Start Time</label>
									  <div class="controls">
										 <input type="text" class="span8 m-wrap" name="start_time" id="start_time" value="<?=$start_time;?>" />
									  </div>
                                  </div>
                                   <div class="control-group">
                                     <label class="control-label">Finish Time</label>
									  <div class="controls">
										 <input type="text" class="span8 m-wrap" name="end_time" id="end_time" value="<?=$end_time;?>" />
									  </div>
                                  </div>
                                  <div class="control-group">
                                     <label class="control-label">Penalties</label>
									  <div class="controls">
										 <textarea class="span12 ckeditor m-wrap description" name="penalties" rows="6" ></textarea>
									  </div>
                                  </div>
                                   <div class="control-group">
                                     <label class="control-label">More Information</label>
									  <div class="controls">
										 <textarea class="span12 ckeditor m-wrap description" name="more_info" rows="6" ></textarea>
									  </div>
                                  </div>
						   ...
						   <div class="form-actions" style="padding-left:10px">
							  ...
							  <button type="button" class="btn" id="cancel" onClick="location.href='processBooking.php?sl=<?=$sl?>'">Cancel</button>
						   </div>
