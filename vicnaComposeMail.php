...
if($_SESSION["userid"]){}else{header('Location: login1.php');exit();}
	...
	if (isset($_POST['send']))
	{
		...
		$mail_body = addslashes($_REQUEST['mail_body']);
		$date = date('Y-m-d H:i:s');
		$insert = "INSERT INTO mailing SET mail_from='".$mail_from."', 
										   mail_to='".$mail_to."',
		...
		$insert1 = "INSERT INTO sent_mailing SET mail_from='".$mail_from."', 
										   mail_to='".$mail_to."',
										   mail_subject = '".$mail_subject."',
										   mail_body = '".$mail_body."',
										   mail_date = NOW(),
		...
		{
			$delete = mysql_query("DELETE FROM mailing_draft WHERE mail_id = '".$_REQUEST['draft_id']."'");
		}
		if($result){
			echo '<script>alert("Mail Send Successfully.");</script>';
			echo '<script>window.location.href="vicnaEmailStuff.php";</script>';
			exit();
			//$msg="<font color='Green'>Successfully Updated.</font>";
		}else{
			$msg="<font color='Red'>Message Not Send!</font>";
		}
	}
	...
	if (isset($_POST['save']))
	{
		...
		$mail_body = addslashes($_REQUEST['mail_body']);
		$date = date('Y-m-d H:i:s');
		$insert = "INSERT INTO mailing_draft SET mail_from='".$mail_from."', 
										   mail_to='".$mail_to."',
										   mail_subject = '".$mail_subject."',
										   mail_body = '".$mail_body."',
										   mail_date = '".$date."'
		...
		$result = mysql_query($insert);
		if($result){
			echo '<script>alert("Mail Save to Draft Successfully.");</script>';
			echo '<script>window.location.href="vicnaEmailStuff.php";</script>';
			exit();
			//$msg="<font color='Green'>Successfully Updated.</font>";
		}else{
		...
		echo "<div style='margin:10px'>$msg_upload</div>";
			  ...
			  ?>
                <div class="col-sm-2" style="background-color:#D2FCFF; min-height:500px;"><br>
                  <!-------------------------------------------------------------------------------->
                  <a title="Create a new Message" style="text-decoration:none;" href="vicnaComposeMail.php"><div class="row" style="padding:5px;  background-color:#CCC;"><span class="glyphicon glyphicon-pencil"></span> Compose</div></a>
                  <a style="text-decoration:none;" href="vicnaEmailStuff.php"><div class="row" style="padding:5px;"><span class="glyphicon glyphicon-inbox"></span> Inbox (<?=$unread;?>)</div></a>
			...
			{
				$fetch = mysql_fetch_array(mysql_query("SELECT * FROM mailing_draft WHERE mail_id = '".base64_decode($_REQUEST['mail_id'])."'"));
			?>
				<div class="col-sm-10" style="padding:10px;"> 
                	<form action="" method="post" enctype="multipart/form-data">
						<input type="hidden" name="draft_id" value="<?=$fetch['mail_id'];?>">
                    	<div class="row" style="margin-bottom:3px;">
                        	<div class="col-sm-2">From </div>
                            <div class="col-sm-10"><input type="email" class="form-control" name="mail_from" value="<?=$fetch['mail_from'];?>"></div>
                        </div>
                        <div class="row" style="margin-bottom:3px;">
                        	<div class="col-sm-2">To </div>
                            <div class="col-sm-10"><input type="email" class="form-control" name="mail_to" value="<?=$fetch['mail_to'];?>"></div>
                        </div>
                        <div class="row" style="margin-bottom:3px;">
                        	<div class="col-sm-2">Subject </div>
                            <div class="col-sm-10"><input style="border: 1px solid #ccc; border-radius: 4px;" type="text" class="form-control" name="mail_subject" value="<?=$fetch['mail_subject'];?>"></div>
                        </div>
                        <div class="row" style="margin-bottom:3px;">
                        	 <div class="col-sm-12"><textarea class="form-control" name="mail_body" id="mail_body" rows="8"><?=stripslashes($fetch['mail_body']);?></textarea></div>
                        </div>
                         <div class="row" style="margin-bottom:3px;">
                        	 <div class="col-sm-12"><input type="submit" name="send" class="btn btn-primary" value="Send">&nbsp;&nbsp;<input type="submit" name="save" class="btn btn-warning" value="Save"> </div>
                        </div>
			...
			?>
                <div class="col-sm-10" style="padding:10px;"> 
                	<form action="" method="post" enctype="multipart/form-data">
                    	<div class="row" style="margin-bottom:3px;">
                        	<div class="col-sm-2">From </div>
                            <div class="col-sm-10"><input type="email" class="form-control" name="mail_from" value="<?=$vicnaMail;?>"></div>
                        </div>
                        <div class="row" style="margin-bottom:3px;">
                        	<div class="col-sm-2">To </div>
                            <div class="col-sm-10"><input type="email" class="form-control" name="mail_to" value=""></div>
                        </div>
                        <div class="row" style="margin-bottom:3px;">
                        	<div class="col-sm-2">Subject </div>
                            <div class="col-sm-10"><input style="border: 1px solid #ccc; border-radius: 4px;" type="text" class="form-control" name="mail_subject" value=""></div>
                        </div>
                        <div class="row" style="margin-bottom:3px;">
                        	 <div class="col-sm-12"><textarea class="form-control" name="mail_body" id="mail_body" rows="8"></textarea></div>
                        </div>
                         <div class="row" style="margin-bottom:3px;">
                        	 <div class="col-sm-12"><input type="submit" name="send" class="btn btn-primary" value="Send">&nbsp;&nbsp;<input type="submit" name="save" class="btn btn-warning" value="Save"> </div>
                        </div>
