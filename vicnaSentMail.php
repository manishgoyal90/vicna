		...
		echo "<div style='margin:10px'>$msg_upload</div>";
			  ...
			  ?>
                <div class="col-sm-2" style="background-color:#D2FCFF; min-height:500px;"><br>
                  ...
                  <a style="text-decoration:none;" href="vicnaSentMail.php"> <div class="row" style="padding:5px; background-color:#CCC;"><span class="glyphicon glyphicon-export"></span>Sent</div></a>
                ...
                <div class="col-sm-10" style="padding:10px;"> 
					<div class="table-responsive" id="tablesec">
						<table id="myTable" class="display table" width="100%" >
							...
							<tbody>
						<?php 
							$sql = "SELECT * FROM sent_mailing WHERE mail_from = '".$row['vicnaEmail']."'";
						...
						?>
								...
								<tr>
									<td><a style="text-decoration:none; color:#000;" href="vicnaSentMailText.php?mail_id=<?=base64_encode($fetch['mail_id']);?>&mail=sent"><?=$fetch['mail_subject'];?></a></td>
									<td><a style="text-decoration:none; color:#000;" href="vicnaSentMailText.php?mail_id=<?=base64_encode($fetch['mail_id']);?>&mail=sent"><?=$fetch['mail_to'];?></a></td>
									<td><a style="text-decoration:none; color:#000;" href="vicnaSentMailText.php?mail_id=<?=base64_encode($fetch['mail_id']);?>&mail=sent"><?=date('D H:i:s', strtotime($fetch['mail_date']));?></a></td>
									<td align="center"><a href="" style="text-decoration:none;" onClick="return deleteone(<?=$fetch['mail_id']?>, <?=$_SESSION['userid']?>);" ><span class="glyphicon glyphicon-trash"></span></a></td>
...
<script>
    ...
    function deleteone(id)
    {
        ...
        if (cnf)
        {
            ...
            $.post('ajax/delsentmail.php', {feedid: id, mode: 'single'},
            function(data)
            {//deletecinematic
					
                $('#tablesec').html(data);
					window.location.href="vicnaSentMail.php";
                //************************************ Table JS ************************************
                /*$('#myTable').dataTable({
                    "aLengthMenu": [
                        [5, 15, 20, -1],
